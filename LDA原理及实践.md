...
具体到我们的问题，我们的所有文档联合起来形成的词向量𝑤⃗是已知的数据，不知道的是语料库主题𝑧⃗的分布。假如我们可以先求出𝑤,z的联合分布𝑝(𝑤⃗ ,𝑧⃗ )，进而可以求出某一个词$w_i$对应主题特征$z_i$的条件概率分布$𝑝(𝑧_𝑖=𝑘|𝑤⃗ ,𝑧⃗_{¬𝑖})$。其中，$𝑧⃗_{¬𝑖}$代表去掉下标为𝑖的词对应的主题后的主题分布。有了条件概率分布$𝑝(𝑧=𝑘|𝑤⃗ ,𝑧⃗_{¬𝑖})$，我们就可以进行Gibbs采样，最终在Gibbs采样收敛后得到第𝑖个词的主题。
